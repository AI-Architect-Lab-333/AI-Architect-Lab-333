...
- **[dgx-spark-idle-llm-profiles](https://github.com/AI-Architect-Lab-333/dgx-spark-idle-llm-profiles)** — systemd **user** profiles on that same DGX Spark: **idle** (GPU free) vs **llm** (the verified pair). Linger no longer reloads ~100 GB of weights on every power button. Idle reboot proven (~2.7 Gi used / ~119 Gi available).
